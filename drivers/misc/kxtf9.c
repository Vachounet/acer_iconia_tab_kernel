...
static int kxtf9_i2c_read(u8 addr, u8 *data, int len)
{
	...
	for (loop_i = 0; loop_i < KXTF9_RETRY_COUNT; loop_i++){
...
int kxtf9_set_bits(int res_index, u8 reg_addr, u8 bits_value, u8 bits_mask)
{
	...
	if(err < 0){
		pr_err("%s write reg_data err\n", __func__);
		goto exit0;
	}
	// Read from device register
	err = kxtf9_i2c_read(reg_addr, &reg_data, 1);
	if(err < 0) {
		pr_err("%s read from register err\n", __func__);
		goto exit0;
	}
	...
	if(err < 0){
		pr_err("%s write to device register err\n", __func__);
		goto exit0;
	}
exit0:
	// Turn on PC1
	...
	if(err1 < 0){
		pr_err("exit write reg_data err\n");
		return err1;
	}
	if(err < 0){
		pr_err("exit err\n");
...
int kxtf9_set_byte(int res_index, u8 reg_addr, u8 reg_value)
{
	...
	err = kxtf9_i2c_read(reg_addr, &reg_data, 1);
	if(err < 0){
		pr_err("%s read from device register err\n", __func__);
		goto exit0;
	}
	// Update resume state data
...
static int kxtf9_verify(void)
{
	...
	pr_info("%s: Chip ID = 0x%02x\n", __FUNCTION__, buf);
	if(err < 0)
		pr_err("read err int source\n");
	if(buf != 1){
		pr_err("%s buff: %d\n", __func__, buf);
		err = -1;
	}
...
static int kxtf9_hw_init(void)
{
	...
	if(err < 0){
		pr_err("%s failed to write TDT_TIMER\n", __func__);
		return err;
	}
	err = kxtf9_i2c_write(INT_CTRL_REG1, &tf9->resume[RES_INT_CTRL_REG1], 1);
	if(err < 0){
		pr_err("%s failed to write INT_CTRL_REG1\n", __func__);
		return err;
	}
	buf[0] = (tf9->resume[RES_CTRL_REG1] | PC1_ON);
	err = kxtf9_i2c_write(CTRL_REG1, buf, 1);
	if(err < 0){
		pr_err("%s failed to write CTRL_REG1\n", __func__);
		return err;
	}
...
int kxtf9_update_g_range(u8 new_g_range)
{
	...
	case KXTF9_G_8G:
		...
		break;
	default:
		pr_err("invalid g range request\n");
		return -EINVAL;
	}
...
static int kxtf9_update_odr(int poll_interval)
{
	...
	pr_info("%s: poll_interval is %dms, ODR set to 0x%02X\n", __FUNCTION__, poll_interval, config);
#endif
...
static int kxtf9_input_init(void)
{
	...
	if(err) {
		pr_err("unable to register input polled device %s: %d\n",
						...
						const char *buf, size_t count)
{
	...
	else		tf9->resume[RES_CTRL_REG1] &= (~WUFE);
	kxtf9_i2c_write(CTRL_REG1, &tf9->resume[RES_CTRL_REG1], 1);
	return count;
}
...
#if defined DEBUG_MSG
	pr_info("%s: [%5d] [%5d] [%5d] [%5d] [%5d] [%5d] [%5d] [%5d] [%5d]\n", __FUNCTION__,\
			xyz[0], xyz[1], xyz[2],\
			0, 0, 0,\
			((int)SENS), ((int)SENS), ((int)SENS));
#endif
						...
						const struct i2c_device_id *id)
{
	...
	if(client->dev.platform_data == NULL) {
		pr_err("platform data is NULL. exiting.\n");
		err = -ENODEV;
		goto err0;
	}
	...
	err = kxtf9_verify();
	if(err < 0) {
		pr_err("unresolved i2c client\n");
		goto err4;
	}

	err = kxtf9_update_g_range(tf9->pdata->g_range);
	if(err < 0) {
		pr_err("update_g_range failed\n");
		goto err4;
	}
...
static int __init kxtf9_init(void)
{
	...
	ret = i2c_add_driver(&kxtf9_driver);
	if (ret != 0) {
